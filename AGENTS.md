...
ライブラリを追加・変更したら、可能な限り verify コードも追加または更新する。

### verify コード配置

- `verify/<judge>/<problem_slug>.cpp` に置く。
- 既存の judge ディレクトリは以下。
    - `verify/library_checker/`
    - `verify/atcoder/`
    - `verify/aizu/`
- verify コードでは対象ライブラリを `#include "math/crt.hpp"` のように include する。
- スクリプトは verify コードから include した hpp を、verify ページの `Target Library` と親ライブラリへの逆リンク生成に使う。

### ライブラリ変更後の再検証

ライブラリや verify コードを変更すると、bundled hash が変わり stale 扱いになる。AC の確認はユーザーがジャッジ上で行う。エージェントは、未確認のまま `verify.py mark` を実行しない。
...
build 時、`scripts/mkdocs_hooks.py` は stale / 未登録の verify を標準エラーへ警告する。新規 hpp に対応する verify が無い、または変更で hash がずれている場合はここで気付ける。
...
- [ ] 可能なら verify コードを追加・更新した。
- [ ] `uv run mkdocs build` が `strict` でエラーなく通り、stale/未登録 verify の警告が出ていない。
