...
from .project_paths import DOCSRC, DOCSRC_LIBRARY, DOCSRC_NOTE
...
def generate_library_index(
    ...
    lines.extend(
        [
            '<small><a href="../site-map/">Site Map</a></small>',
            "",
        ]
    )
...
def generate_site_map(
    ...
    lines = [
        "# Site Map",
        "",
        "検索エンジン向けに、公開ページへのリンクをまとめたページです。",
        "",
        f"<!-- This file is auto-generated by {generator_name} -->",
        "",
        "## Top",
    ...
    lines.extend(["## Verify", "", "- [Verify Top](verify/index.md)", ""])

    verify_groups: dict[str, list[DocEntry]] = {}
    for key, entry in sorted(status.items()):
        parts = key.removeprefix("verify/").split("/")
        if len(parts) < 2:
            continue
        group_name = parts[0].replace("_", " ").title()
        title = entry.get("title", parts[-1].removesuffix(".cpp").replace("_", " ").title())
        doc_path = "verify/" + key.removeprefix("verify/").replace(".cpp", ".md")
        verify_groups.setdefault(group_name, []).append((title, doc_path))

    for group_name, entries in sorted(verify_groups.items()):
    ...
    lines.extend(["## Note", "", "- [Note Top](note/index.md)", ""])
    for title, doc_path in note_entries:
        lines.append(f"- [{title}]({doc_path})")
    lines.append("")

    write_if_changed(DOCSRC / "site-map.md", "\n".join(lines).rstrip() + "\n")
